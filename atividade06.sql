...
create table cidade(idcidade int(2) primary key auto_increment, 
nome varchar(60));


create table estado(idestado int(2) primary key auto_increment, 
nome varchar(2));

create table cliente(idcliente int(2) primary key auto_increment,
nome varchar(60), 
email varchar(60), 
cpf varchar(15), 
data_nasc date, 
idestadoFK int(2),
idcidadeFK int(2),
constraint ClienteEstado foreign key (idestadoFK) references estado(idestado),
constraint ClienteCidade foreign key (idcidadeFK) references cidade(idcidade));

create table categoria (idcategoria int(2) primary key auto_increment, 
nome varchar(40));

create table marca (idmarca int(2) primary key auto_increment, 
nome varchar(30));

create table produto (idproduto int (2) primary key auto_increment, 
idcategoriaFK int(2),
idmarcaFK int(2),
nome varchar(80),
preco float(10,2),
constraint ProdutoCategoria foreign key (idcategoriaFK) references categoria(idcategoria),
constraint ProdutoMarca foreign key (idmarcaFK) references marca(idmarca));

create table vendedor (idvendedor int(2) primary key auto_increment, 
nome varchar(60),
email varchar(60),
cpf varchar(15),
idcidadeFK int(2),
idestadoFK int(2),
constraint VendedorCidade foreign key (idcidadeFK) references cidade(idcidade),
constraint VendedorEstado foreign key (idestadoFK) references estado(idestado));


create table pedido (idpedido int(2) primary key auto_increment,
data_pedido date,
idvendedorFK int(2),
idclienteFK int(2),
constraint PedidoVendedor foreign key (idvendedorFK) references vendedor(idvendedor),
constraint PedidoCliente foreign key (idclienteFK) references cliente(idcliente));

create table itens_pedido (iditens int(2) primary key auto_increment, 
idpedidoFK int(2),
idprodutoFK int(2),
constraint ItensPedido foreign key (idpedidoFK) references pedido(idpedido),
constraint ProdutoPedido foreign key (idprodutoFK) references produto(idproduto),
qtde int, 
...
INSERT INTO produto VALUES (null,2,2,'LG K9',100.50);
